        ...
        async loadData() {
            try {
                ...
                if (res) {
                    ...
                    this.option1.series[0].data = this.getTree(JSON.parse(res.SourceCodeMap), 1)
                ...
                for (const j in tree[i])
                    data2.children.push({
                        ...
                        tooltip: {
                            ...
                            formatter() {
                                if (type === 1) {
                                    const component = new Vue(VueCodeHighlight)
                                    component.language = 'solidity'
                                    let el = Vue.compile(
                                        `<vue-code-highlight language="solidity"><pre style="margin:0">${tree[i][j]}</pre></vue-code-highlight>`
                                    )
                                    el = new Vue({
                                        components: {
                                            VueCodeHighlight
